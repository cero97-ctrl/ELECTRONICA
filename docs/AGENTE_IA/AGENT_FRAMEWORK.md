...
Este documento define un sistema de **arquitectura de tres capas** diseñado para optimizar la interacción entre la inteligencia artificial y la ejecución técnica, separando la lógica probabilística de la **ejecución determinista**. Está pensado para cualquier proyecto que requiera automatización de flujos de trabajo, ya sea desarrollo de software, investigación, análisis de datos, generación de documentos, o cualquier otro dominio que se beneficie de procesos repetibles y verificables.

El marco de trabajo utiliza **directivas estandarizadas** como manuales de operación, una capa de **orquestación inteligente** para la toma de decisiones y scripts de Python especializados para realizar tareas concretas de forma fiable. El objetivo central es **maximizar la fiabilidad** del sistema mediante la validación constante, el manejo de errores autogenerativo y la actualización permanente de los procedimientos basados en el aprendizaje. Al actuar como un puente de **intermediación entre la intención y la implementación**, el agente asegura que la complejidad se gestione a través de herramientas reutilizables y procesos documentados que garantizan resultados consistentes.
...
Esta sección se personaliza por proyecto. A modo de ejemplo, estas son algunas directivas comunes que podrías implementar:

*   **`research_topic.yaml`**: "Investigar sobre un tema determinado y guardar el resultado en un archivo de texto."
*   **`scrape_website.yaml`**: "Extraer el contenido principal de una URL y guardarlo en un archivo."
*   **`git_update.yaml`**: "Gestionar el control de versiones del proyecto y sincronizar con el repositorio remoto."
*   **`system_maintenance.yaml`**: "Realizar diagnóstico y mantenimiento de los recursos del sistema."

Cada proyecto define sus propios flujos de trabajo repetibles en este directorio.
...
Tu papel es conectar **intención e implementación**. Por ejemplo, no raspes webs por ti mismo: consulta la directiva correspondiente, define entradas y salidas, y ejecuta el script apropiado.

### Layer 3: Execution (execution/) — Hacer el trabajo

Scripts deterministas (normalmente en **Python**), cada uno con una sola responsabilidad. Para tareas que requieren entornos específicos (compilación, toolchains), se pueden utilizar contenedores **Docker** para garantizar aislamiento y reproducibilidad.
...
**Deliverables:** resultados destinados al usuario (documentos, informes, archivos generados, subidas a servicios en la nube).
...
Si el proyecto define un mecanismo de notificaciones (alertas audibles, mensajes de sistema, etc.), úsalo para informar al usuario del estado de los flujos:

**Completado:** notificar éxito.

**Esperando entrada:** notificar que se requiere acción del usuario.

**Reglas:**

Notifica siempre antes de quedar bloqueado esperando entrada del usuario.

Notifica también cuando un flujo largo termina correctamente.
...
Para garantizar la reproducibilidad y evitar conflictos de dependencias, se recomienda utilizar un entorno virtual aislado. Esta sección se personaliza según el stack tecnológico del proyecto.

### Ejemplo con Python (Conda/venv):
...
Para tareas que requieren entornos aislados (compilación, toolchains específicas):

```bash
docker build -t <sandbox_image> ./docker/
```
