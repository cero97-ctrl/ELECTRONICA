...
Habiendo creado el skill en la Fase 1, la \textbf{Fase 2} lo utiliza para resolver problemas cuantitativos de ingeniería. El principio fundamental es:
\begin{center}
...
\begin{lstlisting}
[Problema + Skill] --> [Retrieval Embeddings] --> [LLM: Formulador] --> [Oraculo SymPy]
                                                          | (analisis +        |
                                                          |  codigo SymPy)     |
                                                          +---- feedback ------+
                                                                   |
...
\midrule
\textbf{Orquestación} & flujo\_resolver\_skill.py & Valida el problema, consulta al enrutador determinista, invoca el solver y reporta resultados. \\
\midrule
\textbf{Ejecución} & execution/resolver\_skill.py & Motor de retrieval por similitud coseno, formulador LLM, sandbox del oráculo y reflexión. \\
\bottomrule
\end{tabularx}
...
\subsection{1. Retrieval Semántico por Embeddings (0 créditos)}
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Confianza del fundamento} (\texttt{confianza\_retrieval}): clasifica el mejor score coseno en \texttt{alta} ($\ge 0.55$), \texttt{media} ($\ge \text{alerta\_score}$, default $0.35$) o \texttt{baja} ($< 0.35$). Umbrales calibrados con evidencia medida (ver §10.2).
...
\begin{tcolorbox}[tarjetaDato, title={\faIcon{shield-alt}~Detección de Fundamento Débil (Medición Real, 0 créditos)}]
  \begin{tabularx}{\linewidth}{>{\bfseries\color{azulNoche}}l X}
