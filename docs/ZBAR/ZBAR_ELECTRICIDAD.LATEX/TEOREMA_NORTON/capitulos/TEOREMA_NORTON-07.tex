...
\begin{figure}[htbp]
    \begin{subfigure}{0.48\textwidth}
        ...
        \begin{circuitikz}[european, scale=0.7]
            \draw (0,0) node[below] {B} to[battery, l={$V_s$}, invert] (0,4) node[above] {A} to[R, l={$R_1$}] (4,4) node[above] {D};
            \draw (4,4) to[battery, l={$V_2$}] (4,2) to[R, l={$R_2$}] (4,0) node[below] {C};
            \draw (4,4) -- (8,4) node[above] {F};
            \draw (8,4) to[R, l={$R_3$}] (8,2) to[ammeter, l=$I_2$] (8,0) node[below] {G};
            \draw (8,0) -- (4,0) -- (0,0); % Conecta G -> C -> B
        \end{circuitikz}
        \caption{Circuito original.}
        \label{fig:27-6a}
    \end{subfigure}
    \hfill % Espacio horizontal entre las figuras
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{circuitikz}[european, scale=0.7]
            % Copia del circuito (a)
            \draw (0,0) node[below] {B} to[battery, l={$V_s$}, invert] (0,4) node[above] {A} to[ammeter, l=$I_N$] (4,4) node[above] {D};
            \draw (4,4) to[R, l={$R_N$}] (4,0) node[below] {C};
            \draw (4,4) -- (8,4) node[above] {F};
            \draw (8,4) to[R, l={$R_3$}] (8,2) to[ammeter, l=$I_L$] (8,0) node[below] {G};
            \draw (8,0) -- (4,0) to[switch, l=S1] (0,0); % Conecta G -> C -> switch -> B
        \end{circuitikz}
